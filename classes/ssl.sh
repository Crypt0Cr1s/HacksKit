...
case $OS in
  ...
  *) 
   ...
   echo -e "$(ColorRed '#') $(ColorGreen 'TLS supported:')"
	nmap --script ssl-cert -p 443 $1 | grep 'Public Key\|Signature Algorithm'
	nmap -sV --script ssl-enum-ciphers -p 443 $1 | egrep -i 'tls.*:|ssl*:' 
	#nmap -sV --script ssl-enum-ciphers -p 443 $1 | egrep -i 'ssl.*:'
	echo "$(ColorRed '----')"
